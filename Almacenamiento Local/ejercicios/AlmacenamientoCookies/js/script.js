...
// Función para guardar un par de valores en las cookies
...
// Función para obtener las cookies como un objeto
function getCookies() {
    const cookieArray = document.cookie.split('; ').filter(cookie => cookie.includes('='));
    const cookieObj = {};
...
function addRowToTable(name, value) {
    ...
    const row = document.createElement('tr'); row.id = name;
    row.innerHTML = ` <td>${name}</td> 
    <td>${value}</td>
     <td><button onclick="deleteCookie('${name}')">Borrar <span><i class='fa fa-remove'></i></span></button></td>
     <td><button onclick="loadCookieToInputs('${name}', '${value}')">Editar <span style='margin-left: 5px;'><i class='fa fa-edit'></i></span></button></td> `;
    tableBody.appendChild(row);
}

// Función para cargar una cookie en los inputs
function loadCookieToInputs(name, value) {
...
// Función para mostrar todas las cookies en la tabla al cargar la página
function loadCookiesIntoTable() {
...
saveButton.addEventListener('click', () => {
    ...
    if (name && value) {
        if (originalName && originalName !== name) {
            // Si el nombre original es diferente al nuevo, eliminar la cookie original
            deleteCookie(originalName);
        }
...
// Cargar las cookies en la tabla al cargar la página
window.onload = loadCookiesIntoTable;
